# Nexlayer — slim-lake-hadoop

**Live:** [https://awesome-moose-slim-lake-hadoop.cloud.nexlayer.ai](https://awesome-moose-slim-lake-hadoop.cloud.nexlayer.ai)  

**Runtime:** java · **Port:** 8088 · **Deploy branch:** trunk

---

## How this deployment works

**slim-lake-hadoop** is deployed on [Nexlayer](https://nexlayer.ai) — a container-native
platform where every push to `trunk` triggers a fully automated build-and-deploy
...
Triggers on:
- **Push** to `trunk` → production redeploy
...
> The container exposes port 8088. When adding a new service (database, cache,
> worker), add it as a new pod in `nexlayer.yaml` and reference it with
> `${podName:port}` syntax. CI/CD runs on push to `trunk`."*
